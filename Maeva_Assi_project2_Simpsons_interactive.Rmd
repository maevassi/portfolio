...
We will use the [Simpson Dataset Kaggle](https://www.kaggle.com/datasets/prashant111/the-simpsons-dataset/data) which
is composed of `simpsons_characters`, `simpsons_episodes` and `simpsons_script_lines`, respectively containing character details, episode details, and script lines for approximately 600 Simpsons episodes.
...
**a.** define the data;
**b.** define aesthetics (color, shape):
  - define which variables to have on the axes;
**c.** define what kind of plot you want to make;
  - specify how the observations are represented using geometrical objects, points, bars using `geom_boxplot()`, `geom_points()` or `geom_bar()`.
...
## Conclusion
You learned some interesting trivia about the Simpsons:\
- The best rated season of The Simpsons over 2010 to 2016 was season 25 with an average IMDb rating of 6.83 but the median IMDb season rating slightly decreases over the seasons.\
- There are at least three times as many male characters as there are female characters.\
- Homer Simpsons is quite verbose.
